...
- **Điều học được / Đóng góp chính:**
  ...
  - Giới hạn đã phát hiện và khắc phục: ban đầu `drop_latest_records` **không** bị check nào bắt được (chỉ bỏ 5 bài mới nhất thì gate vẫn PASS, freshness vẫn FRESH với 1/19 dòng quá hạn; dấu hiệu duy nhất là `latest_published` lùi từ 2026-07-22 về 2026-06-12). Nhóm đã bổ sung check completeness theo lineage `ExpectColumnUniqueValueCountToBeBetween(paper_id)` (≥ 90% số raw records). Chạy lại riêng kịch bản drop: gate FAIL đúng ở check này (19 < 22 `paper_id`).
